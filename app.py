...
EXCEL_FILE  = "RECLAMAÇÕES FEVEREIRO.xlsx"
...
def to_float(x):
    """Converte valor para float aceitando tanto 1.234,56 quanto 1234.56"""
    s = str(x).strip()
    # Formato BR: 1.234,56
    if re.match(r'^\d{1,3}(\.\d{3})*(,\d{2})?$', s):
        s = s.replace('.', '').replace(',', '.')
    else:
        # Formato US: 1,234.56 ou simples 1234.56
        s = s.replace(',', '')
    try:
        return float(s)
    except:
...
def fmt_brl(valor):
    """Formata float para padrão brasileiro: R$ 127.122,20"""
    try:
        v = float(valor)
        # Formata com separador de milhar . e decimal ,
        s = f"{v:,.2f}"          # 127,122.20  (padrão US)
        s = s.replace(',', 'X').replace('.', ',').replace('X', '.')  # -> 127.122,20
        return f"R$ {s}"
    except:
        return str(valor)

def dev_display(v):
    """Retorna valor formatado BR ou 'NÃO'"""
    if not is_dev(v):
        return "NÃO"
    f = to_float(v)
    if f > 0:
...
def load_data():
    if os.path.exists(EXCEL_FILE):
        ...
        # Garantir todas as colunas (inclusive OBSERVAÇÃO se ainda não existir)
        for i, col in enumerate(COLUNAS):
            if i >= len(df.columns):
                df[col] = ""
            elif df.columns[i] != col:
        ...
        return df
    src = "RECLAMAÇÕES_FEVEREIRO.xlsx"
    if os.path.exists(src):
        raw = pd.read_excel(src)
        raw.columns = raw.iloc[0]; raw = raw[1:].reset_index(drop=True)
        raw.columns = list(raw.columns[:9]) + (["OBSERVAÇÃO"] if len(raw.columns) > 9 else [])
        if "OBSERVAÇÃO" not in raw.columns:
            raw["OBSERVAÇÃO"] = ""
        raw = raw[COLUNAS[:len(raw.columns)]]
        raw.to_excel(EXCEL_FILE, index=False)
        raw["DATA"] = pd.to_datetime(raw["DATA"], errors="coerce")
        return raw
    return pd.DataFrame(columns=COLUNAS)

def save_data(df):
    # Garantir coluna OBSERVAÇÃO antes de salvar
    if "OBSERVAÇÃO" not in df.columns:
        df["OBSERVAÇÃO"] = ""
...
with st.sidebar:
    ...
    st.markdown("---")
    df_sb = load_data()
    st.markdown(f"<div style='color:#7b82a8;font-size:0.8rem'>Base atual</div><div style='color:#e8eaff;font-size:1.4rem;font-weight:700;font-family:JetBrains Mono'>{len(df_sb)}</div><div style='color:#5a6080;font-size:0.75rem'>registros</div>", unsafe_allow_html=True)
...
if pagina == "➕  Novo Registro":
    ...
    if st.button("💾 Salvar Registro", use_container_width=True):
        ...
        if erros:
            ...
            novo = {
                "DATA": pd.Timestamp(data_rec), "CANAL": canal,
                "NOME DO CLIENTE": nome.strip().upper(), "CPF": fmt_doc(cpf_raw),
                "MOTIVO": motivo.strip().upper(), "REPRESENTANTE": representante.strip().upper(),
                "S.A": sa.strip().upper(), "DEVOLUÇÃO": dev_val,
                "RETORNO DO REPRESENTANTE": retorno,
                "OBSERVAÇÃO": observacao.strip()
            }
            df_all = load_data()
            df_all = pd.concat([df_all, pd.DataFrame([novo])], ignore_index=True)
            save_data(df_all)
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    # Formatar total devoluções em BR
    dev_tot_brl = fmt_brl(dev_tot)
    ...
    for col, label, val, val_class, sub in [
        ...
        (k4, "Total Devoluções", dev_tot_brl, "metric-value-md", f"{dev_msk.sum()} casos"),
    ...
    # Formatar devolução em BR para exportação
    df_exp["DEVOLUÇÃO"] = df_exp["DEVOLUÇÃO"].apply(lambda x: fmt_brl(to_float(x)) if is_dev(x) else "NÃO")

    buf = io.BytesIO()
    with pd.ExcelWriter(buf, engine="openpyxl") as writer:
        df_exp.to_excel(writer, index=False, sheet_name="Reclamações")
    buf.seek(0)
    st.download_button(
        label="⬇️ Exportar dados filtrados (.xlsx)",
        data=buf,
...
# ══════════════════════════════════════════════════════════════════════════════
    ...
    with f3: dt_f   = st.date_input("📅 Até", value=None, key="sf")
    ...
    for c in ["_d","_cpf_digits"]:
        if c in res.columns: res = res.drop(columns=[c])

    if not filtrou:
        st.markdown("""<div style="text-align:center;padding:60px 20px;color:#4a5080">
            <div style="font-size:3rem;margin-bottom:16px">🔍</div>
            <div style="font-size:1rem">Use os filtros acima para buscar registros</div>
        </div>""", unsafe_allow_html=True)
    elif res.empty:
        ...
        for orig_idx, row in res.iterrows():
            ...
            obs_html = f"""
                <div style="padding-top:8px;border-top:1px solid #2a2d3e;margin-top:4px">
                    <span style="color:#7b82a8;font-size:0.7rem;text-transform:uppercase;letter-spacing:.06em">Observação</span><br>
                    <span style="color:#c8cde8;font-style:italic">{obs_val if obs_val and obs_val not in ['nan','None',''] else '—'}</span>
                </div>""" if True else ""
            ...
            <div class="search-card">
                ...
                <div style="padding-top:8px;border-top:1px solid #2a2d3e">
            ...
            if st.session_state.get(f"cdel_{orig_idx}", False):
                st.warning(f"⚠️ Tem certeza que deseja excluir o registro de **{row['NOME DO CLIENTE']}**?")
            ...
            if st.session_state.get(edit_key, False):
                ...
                with e6:
                    ...
                    if e_teve_dev:
                        ...
                        e_val_dev = st.number_input("Valor da devolução (R$)", value=ini_val, min_value=0.01, step=0.01, format="%.2f", key=f"ev_{orig_idx}")
                with e7:
                ...
                obs_atual = str(row.get("OBSERVAÇÃO","")).strip()
                if obs_atual in ["nan","None"]: obs_atual = ""
                e_obs = st.text_area("📝 Observação", value=obs_atual, height=80, key=f"eobs_{orig_idx}")
